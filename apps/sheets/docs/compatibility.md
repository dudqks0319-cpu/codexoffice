...
- Sheet management (works on streamed large files too): rename — the workbook.xml entry plus every qualified reference (other sheets' `<f>`/CF/DV formulas, defined-name bodies, chart `c:f` series, internal hyperlink anchors) rewrite together, with Excel quoting rules for the new name; add — a blank worksheet part with its content-type override and relationship, inserted at its on-screen tab position, immediately editable (values, styles, and row/column ops journal into the new part); remove — the part, its rels, its content-type override, and its relationship are dropped, defined names scoped to the sheet die with it, surviving `localSheetId` scopes and the active tab renumber. Hide/show journals workbook visibility and refuses to hide the final visible sheet; reorder rewrites the `<sheets>` order while remapping active-tab and sheet-local name indexes. Removal fails closed while any surviving formula, chart, or defined name references the sheet, when the sheet carries drawings/tables/comments, or when no visible sheet would remain. Adding, removing, hiding, or reordering sheets invalidates calcChain where required, and unexpected workbook mutations fail closed. Duplication follows the separate fully-loaded/source-part restrictions above.
...
- Page Layout and print: orientation, paper size, normal/wide/narrow margins, scale or fit-to-page, gridlines/headings, print area, repeating title rows, header/footer, and freeze state journal per sheet and save into worksheet/workbook OOXML. Export PDF renders the active sheet through a bounded HTML print model (50,000-cell maximum) with those settings. Pixel-identical pagination across font installations and printer drivers is not claimed.
...
External XLSX files stream worksheet dimensions, scalar values, shared and inline strings, booleans, formulas, cached formula values, and source style indices while remaining editable through the journaled save path above. Fonts (including underline and strikethrough), fills, borders, alignment (including wrap text), and the full ECMA-376 built-in number-format table are mapped into the worksheet renderer. RGB, indexed-palette, and theme+tint colors are resolved in the sidecar (theme1.xml clrScheme with the Excel index swap and HSL luminance tint). Merged cells, frozen panes, hidden rows and columns, custom row heights, hidden sheets, sheet tab colors, per-sheet gridline visibility, and default column widths are applied to the rendered worksheet. Rich-text runs in shared strings render as per-run styled cell text. Hyperlink cells (external and internal) render with link styling. Conditional formatting — cellIs operators, text operators, blanks, duplicate/unique, top10, expression, color scales, and data bars — is evaluated live by the Univer conditional-formatting engine from parsed rules and dxf styles. Worksheet tabs and large sparse ranges are displayed without creating a complete JavaScript workbook snapshot.

Formula calculation has two bounded paths. Workbooks at or below 50,000 declared cells preload into Univer for live recalculation. Larger workbooks remain sparse: formula cells plus their dependency closure (up to 50,000 cells) are pinned for live calculation; when the closure cannot be represented, a resident sidecar engine recalculates supported formulas from journaled edits and overlays the requested results. Unsupported fallback functions retain their file-cached values, repeated engine failures disable the fallback for that session, and saves request a full recalculation from Excel on open. The `sheets-streamed-formula-save` Electron E2E covers edit, live result, surgical save, original formula preservation, and fresh-process reopen.
...
This milestone does not claim native Excel chart or calculation fidelity. Remaining streamed-file gaps include chart themes, secondary axes, trendlines, 3D effects, complex DrawingML shapes, scatter-series range editing, absolute-anchor moves, editing/deleting tables that came from the file, structured-reference formula evaluation, range moves, and complete parity for functions the fallback engine does not implement. These gaps are either preserved, shown read-only, or rejected at the edit/save boundary; ordinary streamed editing and saving are supported as documented above.
