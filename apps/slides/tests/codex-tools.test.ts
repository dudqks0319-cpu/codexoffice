...
describe('Codex slide tool surface', () => {
  it('keeps native image generation and removes unsupported media/cloud deck tools', () => {
    ...
    expect(names).toContain('get_evidence_sources')
    expect(names).toContain('set_slide_evidence')
    expect(names).toContain('verify_evidence_deck')
    expect(names).toContain('refresh_managed_text')
    ...
    expect(skill.systemPrompt).toContain('set_slide_evidence')
