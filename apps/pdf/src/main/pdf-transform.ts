...
function addDrawing(pdfDoc: PDFDocument, page: PDFPage, d: DrawingInput): void {
  ...
  if (d.kind === 'note') {
    ...
    annot.set(PDFName.of('T'), PDFHexString.fromText('Codexoffice'))
    appendAnnot(pdfDoc, page, pdfDoc.context.register(annot))
    return
  }
...
export async function insertPdfBytes(
  ...
  assertEditablePdfBytes(bytes)
  assertEditablePdfBytes(otherBytes)
  ...
  assertEditablePdfDocument(dst)
  assertEditablePdfDocument(src)
  if (dst.getPageCount() + src.getPageCount() > PDF_EDIT_MAX_PAGES) {
...
export async function applySaveRequest(
  ...
  assertEditablePdfDocument(pdfDoc)
...
function assertEditablePdfBytes(bytes: Uint8Array): void {
  if (bytes.byteLength > PDF_MAX_EDIT_SOURCE_BYTES) {
    throw new Error('pdf: file exceeds the 64MB editing safety limit')
  }
}

function assertEditablePdfDocument(pdfDoc: PDFDocument): void {
  if (pdfDoc.getPageCount() > PDF_EDIT_MAX_PAGES) throw new Error('pdf: page count exceeds limit')
  if (pdfDoc.context.enumerateIndirectObjects().length > PDF_MAX_INDIRECT_OBJECTS) {
    throw new Error('pdf: indirect object count exceeds limit')
  }
}
