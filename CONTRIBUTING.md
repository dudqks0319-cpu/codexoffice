...
| `SERPER_API_KEY`                                         | Supply an optional Serper key; search otherwise uses DuckDuckGo        |
...
inline sign-in prompt for the app-private Codex account, and web search falls
back to a keyless backend. The app does not reuse standalone Codex CLI config.
